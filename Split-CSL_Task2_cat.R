...
## ------------------------------------------------------------
## Categorical covariate-specific Split-CSL curves under representative profiles
## ------------------------------------------------------------
...
## ------------------------------------------------------------
## Compute one categorical Split-CSL summary
## for one chosen profile
## ------------------------------------------------------------
compute_split_curve_cat <- function(v, x_ref, X_all, base_intervals, active_idx, w_hat,
                                    dominant_idx) {
...
print(obs_profile_plot_df_cat)



## ------------------------------------------------------------
## Reorder RIDRETH3 levels for plotting
## ------------------------------------------------------------
...
obs_profile_plot_df_cat <- obs_profile_plot_df_cat %>%
  mutate(
    value = case_when(
      var == "x13" ~ factor(as.character(value), levels = c("Male", "Female")),
      var == "x18" ~ factor(as.character(value), levels = ridreth3_order),
      var == "x14" ~ factor(as.character(value), levels = c("Yes", "No")),
      var == "x15" ~ factor(as.character(value), levels = c("Yes", "No", "Borderline")),
      var == "x16" ~ factor(as.character(value), levels = c("Yes", "No")),
      var == "x17" ~ factor(as.character(value), levels = c("Yes", "No")),
      TRUE ~ factor(as.character(value))
    )
  )


## ------------------------------------------------------------
## Facet plot for categorical Split-CSL summaries
## ------------------------------------------------------------
p_obs_profiles_cat <- ggplot(
  obs_profile_plot_df_cat,
  aes(x = value, y = pred)
) +
  geom_errorbar(
  ...
  geom_point(aes(colour = "Point prediction"), size = 2.2) +
