...
## ------------------------------------------------------------
## Categorical covariate-specific Full-CSL curves under representative profiles
## ------------------------------------------------------------

## Labels for the categorical covariates
cat_var_labels <- c(
  x13 = "RIAGENDR",
  x18 = "RIDRETH3",
  x14 = "MCQ160L",
...
## Order used in the final plot
cat_var_order <- c("x13", "x18", "x14", "x15", "x16", "x17")
cat_label_order <- cat_var_labels[cat_var_order]

vars_cat <- cat_var_order

X_all_cat <- dataset[, all_x_vars, drop = FALSE]
...
## Align factor levels with the full categorical design matrix
for (v in vars_cat) {
  x_ref_low[[v]]  <- factor(as.character(x_ref_low[[v]]),  levels = levels(X_all_cat[[v]]))
  x_ref_mid[[v]]  <- factor(as.character(x_ref_mid[[v]]),  levels = levels(X_all_cat[[v]]))
  x_ref_high[[v]] <- factor(as.character(x_ref_high[[v]]), levels = levels(X_all_cat[[v]]))
}
...
## ------------------------------------------------------------
## Compute one categorical full-CSL summary
## for one chosen profile
## ------------------------------------------------------------
compute_full_curve_csl_cat <- function(v, x_ref, X_all, X_train, y_train,
  ...
  for (t in seq_along(levs)) {
    
    cat(t)
    
    ## Replace only the selected categorical covariate
    newX <- x_ref
    newX[[v]] <- factor(levs[t], levels = levels(X_all[[v]]))
...
obs_profile_plot_df_cat_full <- obs_profile_plot_df_cat_full %>%
  mutate(
    pred = exp(pred),
    L = exp(L),
    U = exp(U),
...
obs_profile_plot_df_cat_full <- obs_profile_plot_df_cat_full %>%
  mutate(
    short_label = factor(cat_var_labels[var], levels = cat_label_order),
    profile = factor(
      profile,
      levels = c("High observed profile", "Middle observed profile", "Low observed profile")
    ),
    value = case_when(
      var == "x13" ~ factor(value, levels = levels(X_all_cat$x13)),
      var == "x18" ~ factor(value, levels = levels(X_all_cat$x18)),
      var == "x14" ~ factor(value, levels = levels(X_all_cat$x14)),
      var == "x15" ~ factor(value, levels = levels(X_all_cat$x15)),
      var == "x16" ~ factor(value, levels = levels(X_all_cat$x16)),
      var == "x17" ~ factor(value, levels = levels(X_all_cat$x17)),
      TRUE ~ factor(value)
    )
  )

print(obs_profile_plot_df_cat_full)

## ------------------------------------------------------------
## Faceted plot of categorical covariate-specific Full-CSL interval curves
## ------------------------------------------------------------

p_obs_profiles_full_cat <- ggplot(
  obs_profile_plot_df_cat_full,
  aes(x = value, y = pred)
) +
  geom_errorbar(
    aes(ymin = L, ymax = U, colour = "Prediction interval"),
    width = 0.15,
    linewidth = 0.9
  ) +
  geom_point(
    aes(colour = "Point prediction"),
    size = 2.2
  ) +
  facet_grid(profile ~ short_label, scales = "free_x", space = "free_x") +
  ...
  theme(
    ...
    axis.text        = element_text(colour = "black", angle = 25, hjust = 1),
    panel.spacing    = unit(0.6, "lines"),
    legend.position  = "bottom"
  )

print(p_obs_profiles_full_cat)
