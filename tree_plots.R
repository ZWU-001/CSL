...
## ------------------------------------------------------------
## RF variable importance and regression tree
## ------------------------------------------------------------

# Extract the fitted RF model from base_intervals
rf_final <- fits_full[[4]]
...
var_labels <- c(
  ...
  x13 = "Sex (RIAGENDR)",
  x14 = "Liver Condition (MCQ160L)",
  x15 = "Diabetes Status (DIQ010)",
  x16 = "High Blood Pressure (BPQ020)",
  x17 = "Prescription Medicine Use (RXQ033)",
  x18 = "Race/Ethnicity (RIDRETH3)"
)

rf_imp_df <- data.frame(
  var = rownames(rf_imp),
  IncNodePurity = rf_imp[, "IncNodePurity"],
  row.names = NULL
) %>%
...
p_imp <- ggplot(rf_imp_df, aes(x = reorder(label, IncNodePurity), y = IncNodePurity)) +
  geom_col(fill = "grey") +
  coord_flip() +
  xlab(NULL) +
  ylab("IncNodePurity") +
...
## ------------------------------------------------------------
## Surrogate regression tree on the original response scale
## ------------------------------------------------------------
X_train_named <- X_train
colnames(X_train_named) <- c(
  "Albumin", "TotProtein", "UricAcid", "Triglycerides",
  "Cholesterol", "Calcium", "Phosphorus", "Age",
  "BMI", "HbA1c", "Waist", "SysBP",
  "Sex", "LiverCond", "Diabetes", "HighBP", "RxMeds", "Race"
)

## Shorten race/ethnicity labels for the plot
levels(X_train_named$Race) <- c("MexAm", "OthHisp", "White", "Black", "Asian", "Multi")

tree_fit <- rpart(
  y_raw ~ .,
  data = data.frame(X_train_named, y_raw = exp(y_train))
)
