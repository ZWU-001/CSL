...
## Training proportions within the training-calibration set
split_ratios <- c(0.7, 0.6, 0.5, 0.4, 0.3, 0.2)

## Storage for sensitivity-analysis results
results_list <- list()

## ------------------------------------------------------------
## Fix the overall train-calibration-test split
## ------------------------------------------------------------
set.seed(66666)
...
## Test set is fixed at 10%
n3 <- floor(0.1 * n)
idx3 <- split_indices[(n - n3 + 1):n]
...
## Remaining observations are split into training and calibration
remaining_indices <- split_indices[1:(n - n3)]
n_remaining <- length(remaining_indices)

## ------------------------------------------------------------
## Sensitivity analysis over training-calibration split ratios
## ------------------------------------------------------------
for (ratio in split_ratios) {
  
  cat("\n\n========================================================\n")
  cat("SPLIT RATIO:", ratio * 100, ":", (1 - ratio) * 100, "(Train:Calib)\n")
  cat("========================================================\n")
  
  ## Split remaining data according to the current ratio
  n1 <- floor(n_remaining * ratio)
  n2 <- n_remaining - n1
  
  idx1 <- remaining_indices[1:n1]
  idx2 <- remaining_indices[(n1 + 1):n_remaining]
  ...
  cat("Training size:", length(y_train), "\n")
  cat("Calibration size:", length(y_calib), "\n")
  cat("Test size:", n_test, "\n")
  
  ## Objects for storing interval results
  ...
  cat("\nSL Weights:\n")
  names(w_hat) <- c("lm", "gam", "nnet", "rf", "lasso", "gamlss")
  print(round(w_hat, 4))
  ...
  if (length(dominant_idx) == 1) {
    ...
    cat("\nNo dominant learner. Constructing split CP intervals for active learners...\n")
    ...
    for (i in 1:n_test) {
      ...
      endpoints <- c(min(endpoints) - range_extension, endpoints, max(endpoints) + range_extension)
  ...
  ## Empirical coverage and mean interval width on the original scale
  cover_test <- (y_test_raw >= L_final_raw) & (y_test_raw <= U_final_raw)
  width_test <- U_final_raw - L_final_raw
  
  coverage <- mean(cover_test, na.rm = TRUE)
  mean_width <- mean(width_test, na.rm = TRUE)
  
  cat("\n--- Results for", ratio * 100, ":", (1 - ratio) * 100, "split ---\n")
  cat("Coverage:", round(coverage, 3), "\n")
  cat("Mean width:", round(mean_width, 4), "\n")
  cat("Multi-interval count:", multi_interval_count, "\n")
  ...
  results_list[[as.character(ratio)]] <- list(
    ...
    multi_interval_count = multi_interval_count
  )
}
...
summary_df <- data.frame(
  Split = paste0(split_ratios * 100, ":", (1 - split_ratios) * 100),
  N_Train = sapply(results_list, function(x) x$n_train),
  N_Calib = sapply(results_list, function(x) x$n_calib),
  Coverage = sapply(results_list, function(x) round(x$coverage, 3)),
  Mean_Width = sapply(results_list, function(x) round(x$mean_width, 4)),
  Multi_Intervals = sapply(results_list, function(x) x$multi_interval_count)
)
rownames(summary_df) <- NULL

print(summary_df)

## Table of Super Learner weights across split ratios
cat("\n\nSL Weights by Split Ratio:\n")
weights_df <- do.call(rbind, lapply(results_list, function(x) round(x$weights, 4)))
rownames(weights_df) <- paste0(split_ratios * 100, ":", (1 - split_ratios) * 100)
colnames(weights_df) <- c("lm", "gam", "nnet", "rf", "lasso", "gamlss")
print(weights_df)
